...
class ActionCondEffectConvertible(Generic[UT, PT], ABC):
    ...
    def _units_common_buffs(
        ...
        return [
            ...
            # ATK SPD
            self.to_param_up(BuffParameter.ATK_SPD, action_cond.buff_atk_spd, action_cond, payload),
            # FS SPD
    ...
    def to_buff_units(self, action_cond: "ActionConditionEntry", payload: Optional[PT] = None) -> list[UT]:
        """Convert ``action_cond`` to a list of effect units."""
        return [
            unit for unit in
            self._units_common_buffs(action_cond, payload) + self._units_defensive_buffs(action_cond, payload)
