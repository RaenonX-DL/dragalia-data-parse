...
class BuffCountEntry(MasterEntryBase):
    ...
    def get_effective_action_condition_count(self, cond_comp: ConditionComposite) -> int:
        ...
        action_cond_cat = ConditionCategories.get_category_action_condition(cond_comp.action_cond_id)
        ...
        if not isinstance(action_cond_conv, int):
            # Extracted and converted condition is not an integer
            raise AppValueError(
    ...
    def get_buff_up_rate(self, cond_comp: ConditionComposite) -> float:
        """Get the effective buff up rate according to the given condition."""
        rate: float = 0
