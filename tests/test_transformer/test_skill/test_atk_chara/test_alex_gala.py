...
def test_s2_base(transformer_skill: SkillTransformer):
    ...
    assert skill_data.total_mod == pytest.approx([4.97, 5.53])
    assert skill_data.total_mod_at_max == pytest.approx(5.53)
    assert skill_data.mods == approx_matrix([[4.97], [5.53]])
...
def test_s1_has_buff(transformer_skill: SkillTransformer):
    ...
    assert skill_data.total_mod_at_max == pytest.approx(2.02 * 3 + 5.335)
    assert skill_data.mods == approx_matrix([
        [1.61] * 3 + [4.235],
        [1.80] * 3 + [4.741],
        [2.02] * 3 + [5.335]
    ])
    assert skill_data.mods_at_max == pytest.approx([2.02] * 3 + [5.335])
    assert skill_data.max_level == 3
...
def test_s2_has_buff(transformer_skill: SkillTransformer):
    ...
    assert skill_data.total_mod_at_max == pytest.approx(5.53 + 4.862)
    assert skill_data.mods == approx_matrix([
        [4.97, 4.367],
        [5.53, 4.862]
    ])
    assert skill_data.mods_at_max == pytest.approx([5.53, 4.862])
    assert skill_data.max_level == 2
...
def test_s1_break(transformer_skill: SkillTransformer):
    ...
    assert skill_data.total_mod_at_max == pytest.approx(2.35 * 3 + 6.58)
    assert skill_data.mods == approx_matrix([
        [1.94] * 3 + [5.35],
        [2.14] * 3 + [5.90],
        [2.35] * 3 + [6.58],
    ])
    assert skill_data.mods_at_max == pytest.approx([2.35] * 3 + [6.58])
    assert skill_data.max_level == 3
    ...
    assert skill_data.total_mod_at_max == pytest.approx(3.525 * 3 + 9.870)
    assert skill_data.mods == approx_matrix([
        [2.910] * 3 + [8.025],
        [3.210] * 3 + [8.850],
        [3.525] * 3 + [9.870],
    ])
    assert skill_data.mods_at_max == pytest.approx([3.525] * 3 + [9.870])
    assert skill_data.max_level == 3
...
def test_s2_break(transformer_skill: SkillTransformer):
    ...
    assert skill_data.total_mod_at_max == pytest.approx(6.48 * 2)
    assert skill_data.mods == approx_matrix([
        [5.93] * 2,
        [6.48] * 2,
    ])
    assert skill_data.mods_at_max == pytest.approx([6.48] * 2)
    assert skill_data.max_level == 2
    ...
    assert skill_data.total_mod_at_max == pytest.approx(9.720 * 2)
    assert skill_data.mods == approx_matrix([
        [8.895] * 2,
        [9.720] * 2
    ])
    assert skill_data.mods_at_max == pytest.approx([9.720] * 2)
    assert skill_data.max_level == 2
